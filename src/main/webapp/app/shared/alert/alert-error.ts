...
export class AlertError implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('inventoryCapstoneApp.error', (response: EventWithContent<unknown>) => {
      const errorResponse = (response as EventWithContent<AlertErrorModel>).content;
      this.addErrorAlert(errorResponse.message);
    });

    this.httpErrorListener = this.eventManager.subscribe('inventoryCapstoneApp.httpError', (response: EventWithContent<unknown>) => {
